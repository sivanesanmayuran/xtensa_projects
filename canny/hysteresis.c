	...
	float tlow, float thigh, unsigned char *edge)
{
   ...
   short int sumpix;
   int maximum_mag;
   int hist[32768] __attribute__((aligned(16)));
#else
   ...
   ****************************************************************************/
#ifdef USE_HW
   unsigned int zero[4] __attribute__((aligned(16))) = {0x00,0x00,0x00,0x00};
   REG_SIMD data = ld_REG_SIMD((REG_SIMD*)&zero[0], 0);
   for(r=0;r<8192;r++)
	   SIMD_STORE(&hist[r*4],0 , data);
#else
   ...
   ****************************************************************************/
   for(r=1,numedges=0;r<32768;r++){//SM -NONMAXSUP
#ifdef USE_HWNG
	  NONMAXSUP(&hist[0],r, numedges /*inout*/, maximum_mag /*inout*/);
#else
