...
#include <stdio.h>
#include "ecc.h"
#include <xtensa/tie/HW_RScode.h>
...
{	
  int n, L, L2, k, d, i;
  int __attribute__((aligned(16))) psi[MAXDEG], psi2[MAXDEG], D[MAXDEG];
  int __attribute__((aligned(16))) gamma[MAXDEG];
...
#ifdef HW//SMOK
      /* psi = psi2 */
      HW_STORE((REG_SIMD*)psi, 0, HW_LOAD(psi2, 0));
      HW_STORE((REG_SIMD*)&psi[4], 0, HW_LOAD(&psi2[4], 0));
#else
...
#ifdef HW//SMOK
      /* psi = psi2 */
      HW_STORE((REG_SIMD*)Lambda, 0, HW_LOAD(psi, 0));
      HW_STORE((REG_SIMD*)&Lambda[4], 0, HW_LOAD(&psi[4], 0));
#else
...
compute_modified_omega ()
{
  ...
  zero_poly(Omega);
#ifdef HW//SMOK
  HW_STORE((REG_SIMD*)Omega, 0, HW_LOAD(product, 0));
#else
...
{
  int i, j;
  int __attribute__((aligned(16))) tmp1[MAXDEG*2];
#ifdef HW//SMOK
	unsigned int b[4] __attribute__((aligned(16))) = {0x00,0x00,0x00,0x00};
	//REG_SIMD zerodata = HW_LOAD(b, 0);
	HW_STORE((REG_SIMD*)dst, 0, HW_LOAD(b, 0));
	HW_STORE((REG_SIMD*)&dst[4], 0, HW_LOAD(b, 0));
#else
  for (i=0; i < (MAXDEG*2); i++) dst[i] = 0;
#endif
  for (i = 0; i < MAXDEG; i++) {
#ifdef HW//SMOK
	  HW_STORE((REG_SIMD*)&tmp1[MAXDEG], 0, HW_LOAD(b, 0));
	  HW_STORE((REG_SIMD*)&tmp1[12], 0, HW_LOAD(b, 0));
#else
...
#ifdef HW//SMOK
    REG_SIMD in1 = HW_LOAD((REG_SIMD*)&dst[0], 0);
    REG_SIMD in2 = HW_LOAD((REG_SIMD*)tmp1, 0);
    RS_ADDPOLY(in1,in2);
    HW_STORE( (REG_SIMD*)&dst[0], 0,in1);
    in1 = HW_LOAD((REG_SIMD*)&dst[4], 0);
    in2 = HW_LOAD((REG_SIMD*)&tmp1[4], 0);
    RS_ADDPOLY(in1,in2);
    HW_STORE( (REG_SIMD*)&dst[4], 0,in1);
    //RS_ADDPOLY((REG_SIMD*)&dst[0] /*inout*/, (REG_SIMD*)tmp1);
    //RS_ADDPOLY((REG_SIMD*)&dst[4] /*inout*/, (REG_SIMD*)&tmp1[4]);
#else
...
{
  int __attribute__((aligned(16))) e, tmp[MAXDEG];
  ...
  for (e = 0; e < NErasures; e++) {
    copy_poly(tmp, gamma);
#ifdef HW
    scale_poly(RS_GEXP(ErasureLocs[e]), tmp);//
#else
...
#ifdef HW//SMOK
  REG_SIMD in1 = HW_LOAD((REG_SIMD*)&dst[0], 0);
  REG_SIMD in2 = HW_LOAD((REG_SIMD*)&src[0], 0);
  RS_ADDPOLY(in1,in2);
  HW_STORE((REG_SIMD*)&dst[0], 0,in1);

  in1 = HW_LOAD((REG_SIMD*)&dst[4], 0);
  in2 = HW_LOAD((REG_SIMD*)&src[4], 0);
  RS_ADDPOLY(in1,in2);
  HW_STORE((REG_SIMD*)&dst[4], 0,in1);
#else
  for (i = 0; i < MAXDEG; i++)
...
{
#ifdef HW//SMOK
	HW_STORE((REG_SIMD*)dst, 0, HW_LOAD((REG_SIMD*)src, 0));
	HW_STORE((REG_SIMD*)&dst[4], 0, HW_LOAD((REG_SIMD*)&src[4], 0));
#else
  int i;
...
{
#ifdef HW //SMOK
	unsigned int b[4] __attribute__((aligned(8))) = {0x00,0x00,0x00,0x00};
	//REG_SIMD zerodata = HW_LOAD(b, 0);
	HW_STORE((REG_SIMD*)poly, 0, HW_LOAD(b, 0));
	HW_STORE((REG_SIMD*)&poly[4], 0, HW_LOAD(b, 0));
#else
  int i;
...
Find_Roots (void)
{
  ...
  for (r = 1; r < 256; r++) {
    ...
    for (k = 0; k < NPAR+1; k++) {
#ifdef HW
      sum ^= gmult(RS_GEXP((k*r)%255), Lambda[k]);//
#else
			 ...
			 int erasures[])
{
  ...
  if ((NErrors <= NPAR) && NErrors > 0) { 
    ...
    for (r = 0; r < NErrors; r++) {
      ...
      for (j = 0; j < MAXDEG; j++) 
#ifdef HW
    	  num ^= gmult(Omega[j], RS_GEXP(((255-i)*j)%255));
#else
...
#ifdef HW
	denom ^= gmult(Lambda[j], RS_GEXP(((255-i)*(j-1)) % 255));//
#else
