...
int __attribute__((aligned(16))) genPoly[MAXDEG*2];
...
decode_data(unsigned char data[], int nbytes)
{
  ...
  for (j = 0; j < NPAR;  j++) {
    ...
    for (i = 0; i < nbytes; i++) {
#ifdef HW
    	sum = data[i] ^ gmult(RS_GEXP(j+1), sum);
#else
...
#ifdef HW
    RS_GLOG(gmult(synBytes[i], ginv(synBytes[i+1]))));
#else
...
compute_genpoly (int nbytes, int genpoly[])
{
  int __attribute__((aligned(16))) i, tp[256], tp1[256];
