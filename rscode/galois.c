...
{	
#if defined(HW_TABLE) || defined(SW)
...
init_exp_table (void)
{
...
#ifdef HW_TABLE
  unsigned int b1[4] __attribute__((aligned(16))) = {0x01,0x00,0x00,0x00};
  unsigned int b2[4] __attribute__((aligned(16))) = {0x00,0x00,0x00,0x00};
  REG_SIMD data1 = ld_REG_SIMD((REG_SIMD*)&b1[0], 0);
  REG_SIMD data2 = ld_REG_SIMD((REG_SIMD*)&b2[0], 0);
  int t_gexp;
#else
  ...
  for (i = 1; i < 256; i++) {
#ifdef HW_TABLE
		RS_GALOIS_1(data1 /*inout*/, data2 /*inout*/,t_gexp /*out*/);
		gexp[i] = t_gexp;
		gexp[i+255] = t_gexp;
#else
	    pinit = p8;
